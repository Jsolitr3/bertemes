...
const TeamMemberCard = ({ name, primaryImage, children }: TeamMemberCardProps) => {
	return (
		<div className='max-w-sm relative bg-base-100 shadow-lg rounded-lg overflow-hidden'>
			...
			{/* Hover Overlay */}
			<div className='absolute inset-0 backdrop-blur-sm opacity-0 hover:opacity-100 transition-opacity duration-300'>
				{/* Hover Content */}
				<div className='absolute top-0 inset-0 p-4 flex flex-col text-secondary-content overflow-auto '>{children}</div>
			</div>
		</div>
	);
};
