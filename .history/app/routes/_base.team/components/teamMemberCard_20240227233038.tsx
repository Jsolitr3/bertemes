...
const TeamMemberCard = ({ name, primaryImage, children }: TeamMemberCardProps) => {
	return (
		<div className='max-w-sm relative bg-base-100 shadow-lg rounded-lg overflow-hidden'>
			...
			{/* Hover Overlay */}
			<div className='absolute inset-0 bg-gray-800 bg-opacity-50 opacity-0 hover:opacity-100 transition-opacity duration-300 overflow-auto'>
				{/* Hover Content */}
				<div className='absolute inset-0 px-4 flex flex-col items-center justify-center text-secondary-content'>
					{children}
				</div>
			</div>
		</div>
